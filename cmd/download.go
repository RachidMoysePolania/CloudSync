...
var info, errx *log.Logger = helpers.Logger()
			...
			for _, archivo := range files {
				...
				if filename[len(filename)-1] != "" {
					...
					if err != nil {
						...
						errx.Fatalln(fmt.Sprintf("[x] Error al descargar el archivo %v, fallo con el siguiente error: %v", filename[len(filename)-1], err))
					}
			...
			for _, archivo := range files {
				...
				if filename[len(filename)-1] != "" {
					...
					if err != nil {
						...
						errx.Fatalln("Error al crear el cliente blobstorage", err)
					}
					data, err := recursive.CopyFiles(bucketname, archivo)
					if err != nil {
						log.Println(fmt.Sprintf("[x] Error al descargar el archivo %v, fallo con el siguiente error: %v", filename[len(filename)-1], err))
					...
					if err != nil {
						...
						errx.Fatalln("Error al subir el archivo al blobstorage", err)
					}
					time.Sleep(time.Millisecond * 100)
				}
			}
